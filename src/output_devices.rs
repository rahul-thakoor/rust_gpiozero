...
impl PWMOutputDevice {
    ...
    fn generate_sequence(on_time: f32, off_time: f32, fade_in_time: f32, fade_out_time: f32, fps:f32) -> Vec<(f32,f32)>{
    ...
    pub fn on(&mut self){
        self.stop();
        self.pin.lock().unwrap().pwm_write(100);
    ...
    pub fn blink(&mut self,on_time: f32, off_time: f32, fade_in_time: f32, fade_out_time: f32, n:i32){
        self.stop();

        let fps = 25 as f32;
        let sequence: Vec<(f32,f32)> = Self::generate_sequence(on_time, off_time, fade_in_time, fade_out_time, fps);
        
        let pin = Arc::clone(&self.pin);
        let blinking = self.blinking.clone();
        let pinValue = self.value.clone();
        thread::spawn(move || {
               ...
               let pin = pin.lock().unwrap();
                ...
                for (value,delay) in &sequence{
                    if !blinking.load(Ordering::Relaxed){
                        pin.pwm_write(0);
                        pinValue.store(0, Ordering::Relaxed);
                        break;
                    }
                    pin.pwm_write((value *100 as f32) as i32);
                    pinValue.store((value *100 as f32) as isize, Ordering::Relaxed);  
    ...
    pub fn stop(&mut self){
        self.blinking.clone().store(false, Ordering::Relaxed);
        self.pin.lock().unwrap().pwm_write(0);
    ...
    pub fn value(&self) -> f32{
        (self.value.load(Ordering::Relaxed)) as f32 / 100 as f32
    }
